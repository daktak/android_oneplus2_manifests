...
for n in `grep + .repo/local_manifests/enabled/device-${dev}.patch | grep -v ++ | grep -v PRODUCT | grep -v @@ | grep -v device-${dev}.mk | grep -v wpa | awk '{print $2}'`; do mkdir out/target/common/obj/APPS/${n}_intermediates -p; done
startday=`date +%Y%m%d`
